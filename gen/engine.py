...
""" + foot("../", site)
    ...
    if hubs:
        gcards = "".join(f'<a class="gids-card" href="{esc(h["slug"])}.html"><h3>{esc(h["h1"])}</h3><p>{esc(h["description"])}</p></a>' for h in hubs)
        bc = breadcrumb_ld([("Home", base + "/"), ("Gidsen", base + "/gids/index.html")])
        page = head(f"Gidsen & bespaartips | {site['name']}", "Praktische gidsen over de goedkoopste tickets voor pretparken en dierentuinen in Nederland.",
...
""" + foot("../", site)
    ...
    print(f"Built {1+len(cats)+len(items)+(len(hubs)+1 if hubs else 0)} pages -> {out} | {len(urls)} sitemap urls")


if __name__ == "__main__":
    import sys
    build(sys.argv[1] if len(sys.argv) > 1 else os.path.join(PROJ, "site_new"))
